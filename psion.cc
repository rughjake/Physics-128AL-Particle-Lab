...
void analysis ( char* file, int maxevt = 0 ) {
  ...
  TH1F *h_Jmass = new TH1F ( "h_Jmass", "psion mass with PID (GeV)", 400, 2.9, 3.3 );
  h_Jmass->GetXaxis()->SetTitle("mass (GeV)");

  TH1F *h_J_LorFit = new TH1F ( "h_J_LorFit", "psion mass with fitted Lorentzian", 400, 2.9, 3.3 );
  h_J_LorFit->GetXaxis()->SetTitle("mass (GeV)");
      ...
      for ( int k=0;k<nparticles;k++ ) {
       ...
       // Types of p1 and  p2 are checked.
        ...
        if( charge1==1 && charge2==-1  ) {  // The first is positive, the second is negative.
          if( (pid1==ELECTRON && pid2==ELECTRON) || (pid1==MUON && pid2==MUON) ) { // Both the first and second are pion.
            ...
            h_Jmass->Fill ( mass );  // Store mass in a histogram
            h_J_LorFit->Fill ( mass );
          }
        }
      }
    }
  ...
  double bin1 = h_J_LorFit->FindFirstBinAbove(h_J_LorFit->GetMaximum()*4/5);
  double bin2 = h_J_LorFit->FindLastBinAbove(h_J_LorFit->GetMaximum()*4/5);
  double gamma = h_J_LorFit->GetBinCenter(bin2) - h_J_LorFit->GetBinCenter(bin1);
  double x0 = h_J_LorFit->GetBinCenter(h_J_LorFit->GetMaximumBin());
  double max = h_J_LorFit->GetMaximum();

  TF1 *L = new TF1("L",fitFunction,2.6,3.6,6);
  L->SetParameters(1,1,1,max,gamma,x0);
  h_J_LorFit->Fit("L");
//  h_Jmass->Draw("E");
  h_J_LorFit->Draw("E");
  gPad->Update();
  TPaveStats *st = (TPaveStats*)h_J_LorFit->FindObject("stats");
